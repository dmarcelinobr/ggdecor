...
#'
#' You should [import_sans]() first and also install the fonts on your
...
#' # seminal bar chart
#'
#' update_geom_font_defaults(family=font_sans_bold)
#'
#' count(mpg, class) %>%
#'   ggplot(aes(class, n)) +
#'   geom_col() +
...
theme_sans <- function(
  ...
  subtitle_family=if (.Platform$OS.type == "windows") "Public Sans Light" else "Public Sans Light",
  subtitle_size = 13,
  subtitle_face = "plain",
  subtitle_margin = 15,
  ...
  caption_family=if (.Platform$OS.type == "windows") "Public Sans Light" else "Public Sans Light",
  caption_size = 9,
